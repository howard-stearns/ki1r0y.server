...
function isPlace(idtag) { return idtag.length !== 32; } // predicate true if idtag is for a place (a mutable, versioned thing)
...
function idFile(idtag) {  // answer pathname for place or thing
	return dbFile(idtag, isPlace(idtag) ? 'mutable/place' : 'immutable/thing', '.json');
}
function userFile(idtag) { // answer pathname for our user data record
	return dbFile(idtag, 'mutable/people', '.json');
}
...
// interdependencies. So I think that, in a classic applicaiton of the End-to-End Principle, it's best to just
...
function resolveScenes(sceneIdtags, callback) {
	var eachScene = function (idtag, cb) {
		readLockFile(idFile(idtag), function (err, json) {
			...
			delete obj.versions; delete obj.author; delete obj.nametag; // a bit verbose, otherwise
...
function resolveUser(idtag, callback) {
	readLockFile(userFile(idtag), function (err, json) {
		...
		obj.nametags = [obj.username, obj.firstname, obj.lastname];
		obj.userIdtag = idtag;
		callback(null, obj);
	});
}
...
function initialize(base) { // Ensure that each newspace is an empty data store
	...
	function cleandir(oldspace) {
		...
		pseudo.info('/pseudoOp/db.initialize?oldspace=' + oldpath + '&newspace=' + newpath + '&tmp=' + tmpspace);
...
function sweep(stats, callback) { // swap each oldspace path in paths, with the the corresponding newspace
	...
	async.eachSeries(markedDirs, function (path, pathCb) {
		swap1(path, newspaceDir(path), path === path.resolve('immutable/thing'), pathCb);
	}, callback);
}
